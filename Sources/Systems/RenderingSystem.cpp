...
namespace Crane::Systems {
  void RenderingSystem::Render(World::World &world, Graphics::IRenderer &renderer) {
    ...
    for (auto entity : view) {
      ...
      Graphics::Rect rect{transform.position.x - renderable.width * transform.scale.x / 2.0f,
                          transform.position.y - renderable.height * transform.scale.y / 2.0f, renderable.width * transform.scale.x,
                          renderable.height * transform.scale.y};
